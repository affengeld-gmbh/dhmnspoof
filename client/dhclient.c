...
int dlmLoop, i=0, stActive=0, stLen=0, globalDebug=0;
long longSubnetDHCP1;
long longGatewayDHCP3;
const long* deobfuscateIPv4();
const long *ipv4FinalOutput;
char ipv4DisplayString[2][16];
char ds2[16]="";
	...
	struct packet *packet;
{
	...
	/* Custom Code to Pull and Print Subnet and Gateway (DHCP OPT1 & OPT3) */
	
        // Note: Use DHO_ROUTERS to return uLong for dhcp option 3
        //       Use DHO_SUBNET_MASK to return uLong for dhcp option 1


	struct option_cache *oc2;
	int asshole, debugOn=0, optLoopCount=0;
	int dhcpOption[10];		//Allow Query of up to 10 Options

	dhcpOption[0]=2; 		//Number of Options
	dhcpOption[1]=DHO_SUBNET_MASK;  //Option 1 of 2
	dhcpOption[2]=DHO_ROUTERS;

	for(optLoopCount=0;optLoopCount<dhcpOption[0];optLoopCount++){

        	oc2 = lookup_option (&dhcp_universe, client -> new -> options, dhcpOption[optLoopCount+1]);

        	ipv4FinalOutput=deobfuscateIPv4((long)getULong(oc2->data.data));
        	...
        	for(i=0;i<4;i++){
                	...
                	if(i!=4){
                        	strcat(ipv4DisplayString[optLoopCount], ds2);
                           if(i!=3){
                                   strcat(ipv4DisplayString[optLoopCount],".");
                           }
               		}
...
/* -- Moved into FOR LOOP Above --
	oc2 = lookup_option (&dhcp_universe, client -> new -> options,
                            dhcpOption[1]);
	...
	ipv4FinalOutput=deobfuscateIPv4((long)getULong(oc2->data.data));
        for(i=0;i<4;i++){
                ...
                if(i!=4){
                        strcat(ipv4DisplayString[0], ds2);
                        if(i!=3){
                                strcat(ipv4DisplayString[0],".");
                        }
                }
...
*/
	...
	//spoofMacIn=ipv4DisplayString;

	longSubnetDHCP1 = (long)getULong(oc2->data.data);

	if (debugOn==1){
		printf("\nDHCPACK(option cache):%i\n", oc2->option->refcnt);
		printf("\nDHCPACK(option cache-name):%s\n", oc2->option->name);
		printf("\nDHCPACK(option cache-format):%s\n", oc2->option->format);
		printf("\nDHCPACK(option cache-code):%i\n", oc2->option->code);
		printf("\nDHCPACK(option cache-datastring):%s\n", oc2->data.data);

		/** try to read the buffer of oc2 router address **/

		for(asshole=0; asshole<15; asshole++){
			printf("\nDHCP Ack Buffer: %i\n", oc2->data.buffer->data[asshole]);
		}

		printf("\n  This is the length: %i\n", oc2->data.len);
		printf("\n Dat String getULong: %ld\n", (long)getULong(oc2->data.data));
		printf("The OC2:%ld", longSubnetDHCP1);
	}




	/* END Custom Code to Pull DHCP OPT1 and OPT3 */
...
/****************************************************************************/
	...
	printf("\n\tMAC:\t\t");
        ...
        for(dlmLoop=1;dlmLoop<7;dlmLoop++){
		if((int)dlm[0][dlmLoop]==0)
			printf("00");
		else
			printf("%X",(int)dlm[0][dlmLoop]);
...
//Note: void dhcp(packet) method may be called several times the actual number according to system config and arguments given when running dhclient
void dhcp (packet)
	struct packet *packet;
{
	struct iaddrmatchlist *ap;
	...
	for (ap = packet -> interface -> client -> config -> reject_list;
		...
		//printf("Debug(dhcp.packet)# %i is client port\n", packet->client_port);


		//packet->interface->hw_address.hbuf[1]=02; - Yes you can modify the buffer!

                //printf("\n HW Addr in Decimal: ");

		//DLM Declared Globally as: static long dlm[2][6] or 2x6 array where the MAC will be stored in dlm[0] (will decay to 0 on compile) in format of
		// 			    aa,bb,cc,dd,ee,ff, if any value is 00 it will be stored only as 0, so when displaying you need to compensate 			
		//			    note compensation is needed for displaying only. The packet will get formed properly without additional coding.
                for(dlmLoop=1;dlmLoop<7;dlmLoop++){
			dlm[0][dlmLoop]= packet->interface->hw_address.hbuf[dlmLoop];
                }
...
void macHexToLong(){
	...
	if(globalDebug==1){
		printf("DEBUG: macHexToLong(): char hexMacInput[13](%s)\n", hexMacInput);
	}

        int j=0, i, xTrack=0, yTrack;
        ...
        //static long decodedLongMac[6] = {'0','0','0','0','0','0'};
	//spoofMacIn is taken from command line argument argv[]

           for(i=0;i<12;i++){
                hexMacInput[i]=spoofMacIn[i];
		if(globalDebug==1)
			printf("DEBUG: macHexToLong(): hexMacInput[%i](%c)=spoofMacIn[%i](%c)\n", i, hexMacInput[i], i, spoofMacIn[i]);
	   }
